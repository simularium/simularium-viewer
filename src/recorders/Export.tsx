...
import CanvasRecorder, {
    RecordingOptions,
} from "../colorizer/recorders/CanvasRecorder";
import { AppThemeContext } from "./AppStyle";
import Mp4VideoRecorder, {
    VideoBitrate,
} from "../colorizer/recorders/Mp4VideoRecorder";
...
export default function Export(inputProps: ExportButtonProps): ReactElement {
    ...
    const customRangeFrames = Math.max(
        Math.ceil((customMax - customMin + 1) / frameIncrement),
        1
    );

    const totalFrames =
        rangeMode === RangeMode.CUSTOM ? customRangeFrames : props.totalFrames;
    ...
    const getApproximateVideoFilesizeMb = (): string => {
        ...
        const maxVideoBitsResolution =
            props.getCanvas().width *
            props.getCanvas().height *
            totalFrames *
            compressionRatioBitsPerPixel;

        const sizeInMb =
            Math.min(maxVideoBitsResolution, maxVideoBitsDuration) / 8_000_000; // bits to MB
    ...
    const modalFooter = (
        <VerticalDiv>
            ...
            {errorText && (
                <p style={{ color: theme.color.text.error, textAlign: "left" }}>
                    {errorText}
                </p>
            )}
        </VerticalDiv>
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            <CustomRadio value={RecordingMode.IMAGE_SEQUENCE}>
                                PNG image sequence
                            </CustomRadio>
                            {/* Optional tooltip here in case WebCodecs API is not enabled. */}
                            <Tooltip
                                title={
                                    "Video recording isn't supported by this browser."
                                }
                                open={isWebCodecsEnabled ? false : undefined}
                                trigger={tooltipTrigger}
                            >
                                <CustomRadio
                                    value={RecordingMode.VIDEO_MP4}
                                    disabled={
                                        isRecording || !isWebCodecsEnabled
                                    }
                                >
                                    MP4 video
                                </CustomRadio>
                    ...
                    <Card size="small">
                        ...
                        >
                            <Space direction="vertical">
                                ...
                                <Radio
                                    value={RangeMode.CURRENT}
                                    disabled={
                                        isRecording ||
                                        recordingMode ===
                                            RecordingMode.VIDEO_MP4
                                    }
                                >
                                    Current frame only
                                </Radio>
                                <Radio value={RangeMode.CUSTOM}>
                                    Custom range
                                </Radio>
                    ...
                    <div>
                        ...
                        <div style={{ paddingLeft: "4px" }}>
                            ...
                            <p>
                                ...
                                before downloading&quot; in your browser
                                settings
                            </p>
                            <p>
                                3. For best results, keep this page open while
                                exporting
                            </p>
                        </div>
                    </div>
                    ...
                    <HorizontalDiv style={{ flexWrap: "nowrap" }}>
                        <label style={{ width: "100%" }}>
                            <p>
                                {recordingMode === RecordingMode.IMAGE_SEQUENCE
                                    ? "Prefix:"
                                    : "Filename:"}
                            </p>
                            <Input
