...
import { Box3, Box3Helper, BufferAttribute, BufferGeometry, Color, DirectionalLight, Group, HemisphereLight, LineBasicMaterial, LineSegments, PerspectiveCamera, Raycaster, Scene, Vector2, Vector3, WebGLRenderer, Mesh, Quaternion } from "three";
...
var VisGeometry = /*#__PURE__*/function () {
  ...
  _createClass(VisGeometry, [{
    ...
    value: function loadPdb(pdbName, assetPath) {
      ...
      var pdbmodel = new PDBModel(pdbName);
    ...
    value: function mapIdToGeom(id, meshName, pdbName, assetPath) {
      ...
      var unassignedName = "".concat(VisAgent.UNASSIGNED_NAME_PREFIX, "-").concat(id);
      ...
      if (pdbName && !this.pdbRegistry.has(pdbName) && !this.pdbLoadAttempted.get(pdbName)) {
        ...
        var pdbmodel = new PDBModel(unassignedName);
        pdbmodel.create(1);
      ...
      agents.forEach(function (agentData) {
        ...
        if (visType === VisTypes.ID_VIS_TYPE_DEFAULT) {
          ...
          if (wasHidden || typeId !== lastTypeId || visType !== visAgent.visType) {
            ...
            if (!pdbGeom && !meshGeom) {
              _this9.resetAgentGeometry(visAgent, new Mesh(VisAgent.sphereGeometry));
            }
          ...
          if (visType !== visAgent.visType) {
            var _meshGeom = VisAgent.makeFiber();
          ...
          if (USE_INSTANCE_ENDCAPS) {
            var q = new Quaternion().setFromEuler(visAgent.mesh.rotation);
    ...
    value: function cancelAllAsyncProcessing() {
      ...
      TaskQueue.stopAll(); // signal to cancel any pending pdbs
