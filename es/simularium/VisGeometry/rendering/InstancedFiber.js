...
import { BufferAttribute, BufferGeometry, CylinderGeometry, InstancedBufferAttribute, InstancedBufferGeometry, Mesh, DataTexture, RGBFormat, FloatType, Vector2, Vector3, Group } from "three";
...
function createTubeGeometry() {
  ...
  var radius = 1;
  var length = 1;
  var baseGeometry = new CylinderGeometry(radius, radius, length, numSides, subdivisions, openEnded); // fix the orientation so X can act as arc length
  ...
  geometry.setAttribute("uv", new BufferAttribute(uvArray, 2)); // dispose old geometry since we no longer need it
...
var InstancedFiber = /*#__PURE__*/function () {
  ...
  function InstancedFiber(nCurvePoints, count) {
    ...
    this.shaderSet = createShaders(this.nSegments, this.nCurvePoints); // make typescript happy. these will be reallocated in reallocate()
  ...
  _createClass(InstancedFiber, [{
    ...
    value: function reallocate(n) {
      ...
      newData.set(oldData);
      this.curveData = new DataTexture(newData, this.nCurvePoints, n, RGBFormat, FloatType);
      ...
      this.instancedGeometry = new InstancedBufferGeometry().copy(this.curveGeometry); // install the new geometry into our Mesh object
