...
function getHighlightColor(color) {
  var hiColor = new _three.Color(color);
  var hsl = {
    h: 0,
    s: 0,
    l: 0
...
var VisAgent = /*#__PURE__*/function () {
  ...
  function VisAgent(name) {
    ...
    this.id = NO_AGENT;
    this.visType = _VisTypes.default.ID_VIS_TYPE_DEFAULT;
    this.name = name;
    this.color = new _three.Color(VisAgent.UNASSIGNED_MESH_COLOR);
    this.active = false;
    this.typeId = -1;
    this.colorIndex = 0;
    this.followed = false;
    ...
    this.baseMaterial = new _three.MeshLambertMaterial({
      color: new _three.Color(this.color)
    });
    ...
    this.mesh = new _three.Mesh(VisAgent.sphereGeometry, this.baseMaterial);
    this.mesh.userData = {
      id: this.id
    };
  ...
  _createClass(VisAgent, [{
    ...
    value: function resetMesh() {
      ...
      this.followed = false;
      this.highlighted = false;
      this.setColor(new _three.Color(VisAgent.UNASSIGNED_MESH_COLOR), 0);
    }
  }, {
    key: "resetPDB",
    ...
    value: function assignMaterial() {
      ...
      if (this.mesh instanceof _three.Mesh) {
    ...
    value: function updateFiber(subpoints, collisionRadius, scale) {
      ...
      this.fiberCurve = new _three.CatmullRomCurve3(curvePoints);
      var fibergeometry = new _three.TubeBufferGeometry(this.fiberCurve, 4 * (numPoints - 1), // 4 segments per control point
    ...
    value: function getFollowPosition() {
      if (this.visType === _VisTypes.default.ID_VIS_TYPE_FIBER && this.fiberCurve) {
        return this.fiberCurve.getPoint(0.5);
      } else {
        return new _three.Vector3().copy(this.mesh.position);
      }
    }
  }], [{
    key: "makeFiber",
    value: function makeFiber() {
      ...
      var fiberMesh = new _three.Mesh(geometry);
      fiberMesh.name = "Fiber";
      var fiberGroup = new _three.Group();
      fiberGroup.add(fiberMesh);

      if (!_VisTypes.USE_INSTANCE_ENDCAPS) {
        var fiberEndcapMesh0 = new _three.Mesh(VisAgent.fiberEndcapGeometry);
        fiberEndcapMesh0.name = "FiberEnd0";
        var fiberEndcapMesh1 = new _three.Mesh(VisAgent.fiberEndcapGeometry);
...
exports.default = VisAgent;

_defineProperty(VisAgent, "UNASSIGNED_MESH_COLOR", 0xff00ff);

_defineProperty(VisAgent, "UNASSIGNED_NAME_PREFIX", "Unassigned");

_defineProperty(VisAgent, "sphereGeometry", new _three.SphereBufferGeometry(1, 32, 32));

_defineProperty(VisAgent, "fiberEndcapGeometry", new _three.SphereBufferGeometry(1, 8, 8));

_defineProperty(VisAgent, "followMaterial", new _three.MeshBasicMaterial({
  color: new _three.Color(0.14, 1, 0)
}));
...
_defineProperty(VisAgent, "membraneData", {
  ...
  facesUVScale: new _three.Vector2(40.0, 40.0),
  sidesUVScale: new _three.Vector2(2.0, 40.0)
});