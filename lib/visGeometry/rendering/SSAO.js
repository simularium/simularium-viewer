...
var SSAO1Pass = /*#__PURE__*/function () {
  function SSAO1Pass(radius, threshold, falloff) {
    (0, _classCallCheck2["default"])(this, SSAO1Pass);
  ...
  (0, _createClass2["default"])(SSAO1Pass, [{
    key: "resize",
    ...
    value: function createNoiseTex() {
      ...
      var tex = new _three.DataTexture(noisedata, 4, 4, _three.RGBAFormat, _three.FloatType);
    ...
    value: function createSSAOSamples() {
      ...
      for (var i = 0; i < 64; i++) {
        ...
        var sample = new _three.Vector3(Math.random() * 2.0 - 1.0, // -1..1
        Math.random() * 2.0 - 1.0, // -1..1
        Math.random() // 0..1
        );
        sample.normalize();
        sample.multiplyScalar(Math.random()); // Uncomment all this to try to get better samples
