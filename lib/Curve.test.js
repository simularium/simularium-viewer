...
function initCubicPolynomial(x0, x1, t0, t1) {
  return new _three.Vector4(x0, t0, -3.0 * x0 + 3.0 * x1 - 2.0 * t0 - t1, 2.0 * x0 - 2.0 * x1 + t0 + t1);
}
...
function sampleCurve(t, points) {
  var point = new _three.Vector3(0, 0, 0);
  var l = points.length;
  var p = (l - 1) * t;
  ...
  var p0 = new _three.Vector3(),
      p3 = new _three.Vector3(); // 4 points (p1 & p2 defined below)

  if (intPoint > 0) {
    p0 = new _three.Vector3().copy(points[(intPoint - 1) % l]);
  } else {
    // extrapolate first point
    //tmp.subVectors( points[ 0 ], points[ 1 ] ).add( points[ 0 ] );
    p0 = new _three.Vector3().copy(points[0]).add(points[0]).sub(points[0]); //(points[0]-points[1]) + points[0];
  }

  var p1 = new _three.Vector3().copy(points[intPoint % l]);
  var p2 = new _three.Vector3().copy(points[(intPoint + 1) % l]);

  if (intPoint + 2 < l) {
    p3 = new _three.Vector3().copy(points[(intPoint + 2) % l]);
  } else {
    // extrapolate last point
    //tmp.subVectors( points[ l - 1 ], points[ l - 2 ] ).add( points[ l - 1 ] );
    p3 = new _three.Vector3().copy(points[l - 1]).add(points[l - 1]).sub(points[l - 2]); //(points[l-1]-points[l-2])+points[l-1];
  ...
  point = new _three.Vector3(calcCubicPolynomial(weight, px), calcCubicPolynomial(weight, py), calcCubicPolynomial(weight, pz));
  return point;
} // ------
...
function createTube(points, t, angle, volume, offset, normal, vT, vB, vN) {
  ...
  vT.copy(new _three.Vector3().copy(next).sub(prev).normalize()); // if next-prev and next+prev are parallel, then
  ...
  var check = new _three.Vector3().copy(vT).dot(new _three.Vector3().copy(next).add(prev).normalize());
  ...
  if (Math.abs(check) < 0.999) {
    ...
    vB.copy(new _three.Vector3().copy(next).add(prev).normalize().cross(vT).normalize());
  } else {
    ...
    var tmpVec = new _three.Vector3().copy(vT).cross(vB).normalize();
    vB.copy(new _three.Vector3().copy(vT).cross(tmpVec).normalize()); // = normalize(cross(T, tmpVec));
  }

  vN.copy(new _three.Vector3().copy(vT).cross(vB)); //-normalize(cross(B, T));
  ...
  var bx = new _three.Vector3().copy(vB).multiplyScalar(circX);
  var ny = new _three.Vector3().copy(vN).multiplyScalar(circY);
  normal.copy(new _three.Vector3().copy(bx).add(ny).normalize()); //xyz = normalize(B * circX + N * circY);
...
function walkCurve(points) {
  ...
  var volume = new _three.Vector2(1, 1); // outputs:
  // pt and normal on the tube centered on our curve:

  var transformed = new _three.Vector3();
  var objectNormal = new _three.Vector3(); // Frenet-Serret frame around this pt on the curve:
  // tangent, binormal, normal

  var T = new _three.Vector3();
  var B = new _three.Vector3();
  var N = new _three.Vector3(); // walk along whole curve
...
describe("Test curve", function () {
  test("on-axis curve computes valid positions", function () {
    var points = [new _three.Vector3(-70, 0, 0), new _three.Vector3(10, 0, 0)];
    walkCurve(points);
  });
  test("on-axis curve computes valid positions", function () {
    var points = [new _three.Vector3(10, 0, 0), new _three.Vector3(-70, 0, 0)];
    walkCurve(points);
  });
  test("very short curve computes valid positions", function () {
    var points = [new _three.Vector3(0.8003046890483816, -0.0012372934219477827, 0.0), new _three.Vector3(-0.7998676465812267, 0.0016908162423625583, 0.0)];
    walkCurve(points);
  });
});