...
exports.default = exports.NO_AGENT = exports.VisGeometry = void 0;
...
var _MembraneShader = _interopRequireDefault(require("./rendering/MembraneShader"));
...
var DEFAULT_BACKGROUND_COLOR = new _three.Color(0.121569, 0.13333, 0.17647);
...
function () {
  function VisGeometry(loggerLevel) {
    ...
    this.materials = [];
    this.desatMaterials = [];
    this.highlightMaterial = new _three.MeshBasicMaterial({
      color: new _three.Color(1, 0, 0)
    });
    ...
    this.sphereGeometry = new _three.SphereBufferGeometry(1, 32, 32);
    this.setupScene();
    this.membrane = {
      ...
      facesMaterial: _MembraneShader.default.MembraneShader.clone(),
      sidesMaterial: _MembraneShader.default.MembraneShader.clone()
    };
    this.membrane.facesMaterial.uniforms.uvscale.value = new _three.Vector2(40.0, 40.0);
    this.membrane.sidesMaterial.uniforms.uvscale.value = new _three.Vector2(2.0, 40.0);
    ...
    this.errorMesh = new _three.Mesh(this.sphereGeometry);
  ...
  _createClass(VisGeometry, [{
    ...
    value: function createMaterials(colors) {
      ...
      for (var i = 0; i < numColors; i += 1) {
        ...
        this.materials.push(new _three.MeshLambertMaterial({
          color: colors[i]
        }));
        var hsl = {
          h: 0,
        ...
        var desatColor = new _three.Color(colors[i]);
        hsl = desatColor.getHSL(hsl);
        desatColor.setHSL(hsl.h, 0.5 * hsl.s, hsl.l);
        this.desatMaterials.push(new _three.MeshLambertMaterial({
          color: desatColor,
          opacity: 0.25,
          transparent: true
        }));
    ...
    value: function createMeshes() {
      ...
      for (var i = 0; i < this.geomCount; i += 1) {
        var runtimeMesh = new _three.Mesh(sphereGeom, materials[0]);
        ...
        var fibercurve = new _three.LineCurve3(new _three.Vector3(0, 0, 0), new _three.Vector3(1, 1, 1));
        var geometry = new _three.TubeBufferGeometry(fibercurve, 1, 1, 1, false);
        var runtimeFiberMesh = new _three.Mesh(geometry, materials[0]);
        ...
        var runtimeFiberEndcapMesh0 = new _three.Mesh(sphereGeom, materials[0]);
        ...
        var runtimeFiberEndcapMesh1 = new _three.Mesh(sphereGeom, materials[0]);
    ...
    value: function resetBounds(boundsAsArray) {
      ...
      this.boundingBox = new _three.Box3(new _three.Vector3(boundsAsArray[0], boundsAsArray[1], boundsAsArray[2]), new _three.Vector3(boundsAsArray[3], boundsAsArray[4], boundsAsArray[5]));
      this.boundingBoxMesh = new _three.Box3Helper(this.boundingBox, BOUNDING_BOX_COLOR);
    ...
    value: function getSphereGeom() {
      ...
      if (!this.meshRegistry.has(sphereId)) {
        this.meshRegistry.set(sphereId, new _three.Mesh(this.sphereGeometry));
      }
      ...
      agents.forEach(function (agentData, i) {
        ...
        if (visType === visTypes.ID_VIS_TYPE_DEFAULT) {
          ...
          for (var j = 0; j < numSubPoints; j += 3) {
            ...
            var y = subpoints[j + 1];
            var z = subpoints[j + 2];
            curvePoints.push(new _three.Vector3(x, y, z));
          }

          var fibercurve = new _three.CatmullRomCurve3(curvePoints);
          var fibergeometry = new _three.TubeBufferGeometry(fibercurve, 4 * numSubPoints / 3, collisionRadius * scale * 0.5, 8, false);
    ...
    value: function animateCamera() {
      ...
      if (this.followObjectIndex !== NO_AGENT) {
        ...
        direction.normalize();
        var newTarget = new _three.Vector3();
    ...
    value: function assignMaterial(runtimeMesh, material) {
      ...
      if (runtimeMesh instanceof _three.Mesh) {
    ...
    value: function addPathForAgentIndex(idx, maxSegments, color) {
      ...
      if (!color) {
        ...
        color = mat && mat.color ? mat.color.clone() : new _three.Color(0xffffff);
      }
      ...
      pathdata.geometry.setAttribute("position", new _three.BufferAttribute(pathdata.points, 3));
      pathdata.geometry.setAttribute("color", new _three.BufferAttribute(pathdata.colors, 3)); // path starts empty: draw range spans nothing

      pathdata.geometry.setDrawRange(0, 0);
      pathdata.line = new _three.LineSegments(pathdata.geometry, pathdata.material);
    ...
    value: function resetAllGeometry() {
      ...
      for (var i = 0; i < MAX_MESHES && i < nMeshes; i += 1) {
        if (this.runTimeMeshes[i].userData) {
          var runtimeMesh = this.setupMeshGeometry(i, this.runTimeMeshes[i], new _three.Mesh(sphereGeom), false);
          this.assignMaterial(runtimeMesh, new _three.MeshLambertMaterial({
            color: 0xff00ff
          }));
        }
      }
